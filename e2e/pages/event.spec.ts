...
test.describe('Event / Pre-Registration Page', () => {
  ...
  test('event page banner placements mount without throwing (data-agnostic)', async ({ page }) => {
    // Empty CMS is a valid state. We only verify slot presence — exact copy
    // assertions break whenever the seed is wiped or admin edits content.
    for (const p of ['announcement_bar', 'popup', 'floating', 'footer_strip']) {
