...
async function main() {
  ...
  const news = [
    ...
    },
    {
      ...
      contentEn: '<p>Selected players will receive CBT access.</p>',
      contentTh: '<p>ผู้เล่นที่ได้รับคัดเลือกจะได้รับสิทธิ์เข้า CBT</p>',
      category: 'ANNOUNCEMENT' as const,
      contentType: 'ANNOUNCEMENT' as const,
      primaryTopicKey: 'getting-started',
  ...
  const [seasonZeroPatchNotes, towerChronicleStarterKit] = await Promise.all([
  ...
  const pageContentMeta: Record<string, { description: string; icon: string; contentEn?: string; contentTh?: string; showInHeader?: boolean; showInFooter?: boolean; headerOrder?: number; footerOrder?: number }> = {
    ...
    download: {
      ...
      contentEn: `<p>Download Eternal Tower Saga on iOS, Android, and PC.</p><p><a href="/download">Get the game</a></p>`,
    },
  }
