<template>
  <main class="min-h-screen bg-[#08070b] text-white">
    ...
    <section class="mx-auto max-w-7xl px-6 pt-24 pb-16">
      ...
      <p class="mt-10 text-xs font-bold uppercase tracking-[0.28em] text-gold">{{ t('news.webzine.topicRegistry') }}</p>
...
interface WebzineLanding { topics?: WebzineTopic[] }

const { t } = useI18n()
const { localized } = useLocalizedField()
const route = useRoute()
const topicKey = computed(() => String(route.params.topicKey || ''))
const { data: landing } = await useFetch<WebzineLanding>('/api/public/webzine/landing')
...
const topic = computed(() => (landing.value?.topics || []).find((item) => item.key === topicKey.value || item.slug === topicKey.value))
const topicLabel = computed(() => localized(topic.value?.labelEn, topic.value?.labelTh) || topicKey.value.replaceAll('-', ' '))
const topicDescription = computed(() => localized(topic.value?.descriptionEn, topic.value?.descriptionTh) || '')
const articles = computed(() => data.value?.data || [])

usePageSeo({
  title: `${topicLabel.value} | ${t('news.webzine.title')}`,
  description: topicDescription.value || t('news.webzine.subtitle'),
})
</script>
